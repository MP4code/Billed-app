...
export default class Login {
  constructor({
    ...
    this.store = store;
    this.usertype = this.localStorage.getItem(key)
      ? JSON.parse(this.localStorage.getItem(key)).type
      : null;
  ...
  handleSubmitEmployee = (e) => {
    ...
    if (!user.email.includes("employee")) {
      alert("Veuillez entrer un email valide pour un employé");
      return;
    }
    this.login(user)
  ...
  // correction : input[data-testid="admin-email-input"]
  handleSubmitAdmin = (e) => {
    e.preventDefault();
    const user = {
      type: "Admin",
    ...
    if (!user.email.includes("admin")) {
      alert("Veuillez entrer un email valide pour un administrateur");
      return;
    }
    this.login(user)
      .catch((err) => console.error("Login failed", err))
